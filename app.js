...
/* 📥 Excel Export (Paid/Due আলাদা Sheet) */
window.exportToExcel = () => {
  if (customers.length === 0) { alert("No data to export"); return; }

  // Paid Sheet
  const paidData = customers.filter(c => c.status === "Paid").map((c,i)=>({
    "SL": i+1,
    "User ID": c.userid,
  ...
  // Due Sheet
  const dueData = customers.filter(c => c.status === "Due").map((c,i)=>({
    "SL": i+1,
    "User ID": c.userid,
    "Customer Name": c.name,
    "Package": c.pkg,
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(dueData), "Due");
